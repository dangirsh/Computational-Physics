...
public:
	...
	_array( const int n1 );					// for 1D vector style
	_array( const int n1, const int n2 );	// for 2D matrix style
	_array( const _array<T> &a );

	//  destructor function
	inline ~_array() { delete [] p; nn=nndim=nn1=nn2=0; }

	// member operations
	inline _array<T>& operator=( const _array<T> &m );
...
template < class T >
_array<T>::_array( const int n1 )			// 1D vector
{
	if( n1 <= 0  ) {
		printf("_array initialized with size = %d, NOT ALLOWED\n", n1 );
		exit( EXIT_FAILURE );
	}
	p = new T [ n1 ];
	if( NULL == p ) {
		printf( "Cannot allocate memory for _array size = %d\n", n1 );
		exit( EXIT_FAILURE );
	}
	nn = nn1 = n1;
	nn2 = 0;
...
template < class T >
_array<T>::_array( const int n1, const int n2 )		// 2D matrix
{
	if( ( n1 <= 0 ) || ( n2 <= 0 ) ) {
		printf("_array initialized with size = %d x %d, NOT ALLOWED\n",
			n1, n2 );
		exit( EXIT_FAILURE );
	}
	p = new T [ n1*n2 ];
	if( NULL == p ) {
		printf( "Cannot allocate memory for _array size = %d x %d\n",
			n1, n2 );
		exit( EXIT_FAILURE );
	}
	nn1 = n1;
...
_array<T>::_array( const _array<T> &a )
{
	p = new T [ a.nn ];
	if( NULL == p ) {
		printf( "Cannot allocate memory for _array size = %d\n", a.nn );
...
template < class T >
void _array<T>::resize( const int n  )		// 1D resize
{
	if( n <= 0  ) {
		printf("_array resize() with size = %d, NOT ALLOWED\n",
			n );
		exit( EXIT_FAILURE );
	}
	delete [] p;
	p = new T [ n ];
	if( NULL == p ) {
		printf( "Cannot allocate memory for _array resize = %d\n",
			n );
		exit( EXIT_FAILURE );
	}
	nn1 = n;
...
template < class T >
void _array<T>::resize( const int n1, const int n2 )		// 2D resize
{
	if( ( n1 <= 0 ) || ( n2 <= 0 ) ) {
		printf("_array resize() with size = %d x %d, NOT ALLOWED\n",
			n1, n2 );
		exit( EXIT_FAILURE );
	}
	delete [] p;
	p = new T [ n1 * n2 ];
	if( NULL == p ) {
		printf( "Cannot allocate memory for _array resize = %d x %d\n",
			n1, n2 );
		exit( EXIT_FAILURE );
	}
	nn1 = n1;
...
template < class T >
_array<T>& _array<T>::operator=( const _array<T> &m )
{
	if( (nn != m.nn) || (nndim != m.nndim)  ){
		printf( "_array = operator invoked with unequal sizes\n"
			"  m1 size = %d, dim= %d\n"
...
template < class T >
inline T& _array<T>::operator()( const int i1, const int i2 )
{
#ifdef ARRAYT_BOUNDS_CHECK
	if( (i1<0) || (i1>=nn1) ||
		(i2<0) || (i2>=nn2) || (nndim != 2 ) ){
		printf( "out of bounds index in _array\n"
			"  size = %d x %d, ndim= %d\n  access = (%d , %d)\n",
			nn1, nn2, nndim, i1, i2 );
		exit( EXIT_FAILURE );
	}
...
template < class T >
inline T& _array<T>::operator()( const int i )
{
#ifdef ARRAYT_BOUNDS_CHECK
	if( (i<0) || (i>=nn) || (nndim != 1 )){
		printf( "out of bounds index in _array\n"
...
template < class T >
inline _array<T>& _array<T>::operator+=( const _array<T>& m  )
{
	if( (m.nn != nn)  ){
		printf( "_array += operator invoked with unequal sizes:\n"
			"   %d and %d\n", nn,  m.n() );

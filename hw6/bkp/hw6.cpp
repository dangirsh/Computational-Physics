...
Compile with: g++ -std=c++0x -O2 -o gen_data hw6.cpp
...
solution::solution(float _h, float omega, float dv_max) :
    h(_h),
    num_its(0),
    I_MAX((Z_MAX - Z_MIN) / _h),
    J_MAX((R_MAX - R_MIN) / _h),
    bound_flags(I_MAX + 1, J_MAX + 1),
    v(I_MAX + 1, J_MAX + 1) {
    set_bound_flags();
    init_voltages();
    solve(omega, dv_max);
...
void solution::set_bound_flags() {
    for_each([ = ](int i, int j) {
        bound_flags(i, j) = on_boundary(i, j);
    });
}

// initializes voltages using the given values at the electrodes and boundary
void solution::init_voltages() {
    for_each([ = ](int i, int j) {
...
float solution::next_v(int i, int j, float omega) {
    float v_old = v(i, j);
    // electrodes & edges are at fixed V
    if (on_boundary(i, j)) return v_old;
    // finite Laplace equation in cylindrical coordinates
    float av;
    if (j == 0) { // special case on axis of symmetry
        av = (1.0 / 6) * (4 * v(i, 1) + v(i + 1, 0) + v(i - 1, 0));
    } else {
        float a = v(i - 1, j), b = v(i, j + 1), c = v(i + 1, j), d = v(i, j - 1);
        av = (1.0 / 4) * (a + b + c + d) + (1.0 / (8 * j)) * (b - d);
    }
    return omega * av + (1.0 - omega) * v_old;
}

// performs SOR with omega until the maximum difference in voltage at any grid
// is less than dv_max. returns the number of iterations ran
...
void solution::debug() {
    std::ofstream f;
    f.open("bounds.txt");
    for_each([ = , &f](int i, int j) {
        f << bound_flags(i, j) << " ";
        if (i == I_MAX) f << std::endl;
    });
}

int main() {
    // float best_omega = part1();
    float best_omega = 1.87;
    part2(best_omega);
    // float h = 0.1; // from part2 data
    // float dv_max = 1; // from part2 data
...
//     part4(h, best_omega, dv_max);
//     part5(h, best_omega, dv_max);
//     float c = part6(h, best_omega, dv_max);
//     printf("%f", c);
// }

// num_its vs. omega
float part1() {
    float h = 0.1, omega = 1, d_omega = 0.1, dv_max = 1, best_omega = 0.0;
    int smallest_num_its = INT_MAX;
    std::ofstream f;
    f.open("omega.dat");
    while (omega < (2 - d_omega)) {
        solution s(h, omega, dv_max);
        if (s.num_its < smallest_num_its) {
            smallest_num_its = s.num_its;
            best_omega = omega;
        }
        f << omega << " " << s.num_its << std::endl;
        omega += d_omega;
    }
    f.close();
    return best_omega;
...
void part2(float omega) {
    ...
    while (dv_max > 0.001) {
        ...
        while (h > 0.05) {
            solution s(h, omega, dv_max);
            float v_orig = s.get_v(0, 0);
            std::cout << h << " " << dv_max << " " << v_orig << std::endl;
            std::cout.flush();
            h /= 2;
        }
        dv_max /= 5;
...
void part5(float h, float omega, float dv_max) {
    ...
    f_v.open("v_contour.dat");//("v_"+fname);
    f_z.open("z_contour.dat");//("z_"+fname);
    f_r.open("r_contour.dat");
    solution s(h, omega, dv_max);
    // select range near electrodes and axis
    int lower_i = (s.I_MAX / 3), upper_i = 2 * (s.I_MAX / 3);
    int lower_j = 0, upper_j = s.J_MAX / 8;
    s.for_each([ = , &f_v, &f_z, &f_r, &s](int i, int j) {
...
float part6(float h, float omega, float dv_max) {
    float eps_0 = 8.8542; // pF/m
    float pi = 4.0 * atan(1.0);
    float field_energy = 0.0;
    solution s(h, omega, dv_max);
    s.for_each([&](int i, int j) {
        float z = s.i_to_z(i), r = s.j_to_r(j);
        float e_z = s.get_e_z(z, r), e_r = s.get_e_r(z, r);
        field_energy += (e_z * e_z + e_r * e_r) * r * h * h;
    });
    field_energy *= pi * eps_0;
    float v_cap = V2 - V1;
    float capacitance = 2 * field_energy / (v_cap * v_cap);
    return capacitance;
}